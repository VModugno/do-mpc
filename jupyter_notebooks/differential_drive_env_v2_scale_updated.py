...
class DifferentialDriveEnvV2(Env):
  ...
  def set_init_position(self,init_position):
    ...
    if isinstance(init_position, list):
      if len(init_position) == 3:
        self.init_position = list(init_position) #do not assign the param because we have to scale it
        #test
        self.init_position[2] /= self.orientation_map
      else:
  ...
  def reset(self):
    ...
    self.duration = self.max_duration
    return np.array(self.state)*np.array([1,1,self.orientation_map]) #return the state unscaled
  ...
  def step(self, action):
    ...
    info = {}
    unscaled_state  = np.array([x,y,theta])
    self.state = np.array([x, y, theta/self.orientation_map])

    self.duration -= 1

    #return self.state, reward, done, info
    return unscaled_state,reward, done, info
  def close(self):
    pass